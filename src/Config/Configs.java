...
///--------------------------ARCHIVO DE CONFIGURACION DEL MENU ADMIN----------------------------------
private static final String CONFIG_FILE2 = "properties/config.properties.admin";

    // Lee el valor de first_run
    public static boolean isFirstRunAdmin() {
        Properties properties = new Properties();
        try (FileInputStream fis = new FileInputStream(CONFIG_FILE2)) {
            properties.load(fis);
            return Boolean.parseBoolean(properties.getProperty("first_run", "true"));
        } catch (IOException e) {
            // Si el archivo no existe, se considera la primera vez
    ...
    // Establece el valor de first_run en el archivo de configuración solo la primera vez
    public static void setFirstRunCompleteAdmin() {
        try {
            // Solo se debe actualizar first_run a false si es la primera ejecución
            if (isFirstRunAdmin()) {
                setFirstRunAdmin(false);  // Cambiar first_run a false
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    // Método para establecer el valor de first_run
    private static void setFirstRunAdmin(boolean value) throws IOException {
        Properties properties = new Properties();
